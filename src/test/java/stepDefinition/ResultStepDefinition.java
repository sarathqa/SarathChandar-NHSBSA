...
public class ResultStepDefinition {
    ...
    public void enterTextInJobTitleField(String keyword) throws IOException {
        try {
            assertTrue(testContext.getPageObjectManager().getResultPage().getResultTitle().getText().contains(keyword));
        } catch (Exception e) {
    ...
    public void sortTheResultWith(String value) {
        CommonUtil.selectByVisibleText(testContext.getPageObjectManager().getResultPage().getSortDropdown(), value);
    }
    ...
    public void listAllTheJobTitle(String keyword) throws IOException {
        ...
        List<WebElement> elementList = testContext.getPageObjectManager().getResultPage().getResultdata();
